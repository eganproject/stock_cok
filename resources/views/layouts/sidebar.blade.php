...
<!-- Brand -->
<div class="nav-brand flex h-16 items-center gap-2.5 border-b border-slate-200 px-5">
    ...
    <div class="nav-brand-text leading-tight">
...
<!-- Navigation -->
<nav class="scrollbar-thin flex-1 space-y-5 overflow-y-auto overflow-x-hidden px-3 py-4">
    <div>
        <p class="nav-section px-3 pb-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Menu Utama</p>
        <div class="space-y-0.5">
            <a href="{{ route('dashboard') }}" title="Dashboard"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('dashboard') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('inventory.index') }}" title="Inventory"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('inventory.*') ? $navActive : $navIdle }}">
                <svg class="h-[18px] w-[18px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.7"><path stroke-linecap="round" stroke-linejoin="round" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9"/></svg>
                <span class="nav-label">Inventory</span>
            </a>

            <a href="{{ route('products.index') }}" title="Master Produk"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('products.*') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('warehouses.index') }}" title="Gudang"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('warehouses.*') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('divisions.index') }}" title="Divisi"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('divisions.*') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('sync.index') }}" title="Status Sync"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('sync.*') ? $navActive : $navIdle }}">
                <svg class="h-[18px] w-[18px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.7"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99"/></svg>
                <span class="nav-label">Status Sync</span>
            </a>

            {{-- Laporan (submenu accordion; saat sidebar kecil → ikon menuju laporan pertama) --}}
            <div class="nav-report-group relative" x-data="{ open: {{ request()->routeIs('reports.*') ? 'true' : 'false' }} }">
                <button type="button" title="Laporan"
                    @click="sidebarCollapsed ? (window.location = '{{ route('reports.restock') }}') : (open = ! open)"
                    class="nav-item group flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('reports.*') ? 'text-slate-900' : $navIdle }}">
                    ...
                    <svg class="nav-chevron h-4 w-4 shrink-0 text-slate-400 transition-transform" :class="open && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5"/></svg>
                </button>

                <div x-show="open" x-transition class="nav-report-sub mt-0.5 space-y-0.5 pl-8">
                    <a href="{{ route('reports.restock') }}"
                        class="block rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('reports.restock') ? $navActive : $navIdle }}">Perlu Restock</a>
                    <a href="{{ route('reports.imbalance') }}"
                        class="block rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('reports.imbalance') ? $navActive : $navIdle }}">Ketimpangan Gudang</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div>
        <p class="nav-section px-3 pb-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Manajemen</p>
        <div class="space-y-0.5">
            <a href="{{ route('users.index') }}" title="Manajemen User"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('users.*') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('roles.index') }}" title="Role"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('roles.*') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('permissions.index') }}" title="Permission"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('permissions.*') ? $navActive : $navIdle }}">
            ...
            <a href="{{ route('profile.edit') }}" title="Pengaturan"
                class="nav-item group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->routeIs('profile.*') ? $navActive : $navIdle }}">
...
<!-- Footer / mini profile -->
<div class="nav-footer border-t border-slate-200 p-3">
    <div class="nav-footer-inner flex items-center gap-3 rounded-lg bg-slate-50 p-2.5">
        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-900 text-xs font-semibold text-white" title="{{ Auth::user()->name }}">
            {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
        </span>
        <div class="nav-footer-text min-w-0 flex-1 leading-tight">
